...
#include "pugixml-1.13/pugixml.cpp"
#include<iostream>
...
Weather XmlService::getWeather(std::string s)
{
	...
	std::cout << "asfasd";
	std::string city = node.child("city").attribute("name").as_string(); //   Êטנמג
	...
	std::string weather = node.child("weather").attribute("value").as_string(); // המזהü
